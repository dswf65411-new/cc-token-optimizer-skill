...
2. **檢查 settings.json** — 確認 model: opusplan、effortLevel: high 設定正確
3. **全域記憶壓縮** — 掃描全機器記憶檔 → 備份 → 平行派工 → 壓縮回報
4. **審查 MCP 伺服器** — 列出可用 CLI 替代的 MCP，建議使用者停用
5. **審查 Subagent 模型設定** — 確認各 subagent 是否指定 model
6. **檢查 Context 使用量** — 確認目前 session context 健康度
...
**不存在**：✅ 告知目前沒有自訂 subagent。

**存在**：讀取每個 `.md` 檔案的 frontmatter，檢查是否有 `model:` 欄位。

對每個沒有指定 model 的 subagent，根據其 `description` 和內容判斷建議：

- 描述包含「搜尋」「讀取」「找」「explore」「search」「read」→ 建議 `model: haiku`
- 描述包含「分析」「重構」「實作」「寫程式」「review」→ 建議 `model: sonnet`
- 描述包含「架構」「設計」「複雜推理」「debug」→ 建議 `model: opus`

列出所有建議，**等使用者確認後再修改檔案**。

---

### 6. 當前 session context 使用狀況
...
> 完成後呼叫 `TaskUpdate` 將任務 6 標為 `completed`。
...
✅ MCP 伺服器           無需調整
⚠️  Subagents           3 個未指定 model（已列出建議，待你確認）
